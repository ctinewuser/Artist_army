...
//
        $this->db->select('U.*,G.genre_name,G.genre_image,'.$subquery.$subquery2);
        $this->db->from('user as U');
        $this->db->join('genre_category as G', 'G.genre_id = U.genre_cat','LEFT');
    ...
    public function songs_list()
        {
            ...
            if($songs){
                $this->response(true,"Songs Fetch Successful",array("songs" => $songs));
            }else{
   ...
   public function add_artistsongs()
   {
        if(!empty($_POST['user_id'])){
          //  $_POST['created_at'] = date('Y-m-d H:i:s');         
            ...
            if(isset($_FILES['songs_image'])){
                 $songs_images = $this->common->do_upload('songs_image','./assets/songs/');
                if(isset($songs_images['upload_data'])){
                    $_POST['songs_image'] = $songs_images['upload_data']['file_name'];
                }
             }
             if(isset($_FILES['songs_name'])){
                 $singer_names = $this->common->do_upload('songs_name','./assets/songs/');
                if(isset($singer_names['upload_data'])){
                    $_POST['songs_name'] = $singer_names['upload_data']['file_name'];
                }
             }
              $post = $this->common->getField('artist_songs',$_POST); 
            $result = $this->common->insertData('artist_songs',$post);
            $postid = $this->db->insert_id();
...
///////// artistsongs_list //////////////////    
    public function artistsongs_list()
    {
        ...
        $songsdata =  $this->common->getData('artist_songs',array('user_id'=>$user_id),array()); 
         if(!empty($songsdata))
        {
...
///////////////// artistgoallist_byid  /////////////////////   
     public function artistgoal_list()
    {
        ...
        $goalsdata =  $this->common->getData('goals',array('user_id'=>$user_id),array()); 
         if(!empty($goalsdata))
        {
            $this->response(true,"Success",array('goals'=>$goalsdata));
    ...
    public function favourite_artistlist()
        ...
        if(!empty($userid))
        {
         $this->db->select('RF.*,U.full_name,U.profile_image,U.email,U.user_type');
    ...
    public function get_allfriends()
    {
        ...
        $data =  $this->common->getData('friends',array('user_id'=>$user_id),array()); 
         if(!empty($data))
        {
        $this->response(true,"Success",array('friends'=>$data));
...
/////////////////////////all_concert/////////////////////
    public function all_concert()
    {
        ...
        $concertdata =  $this->common->getData('concerts',array('user_id'=>$user_id),array()); 
...
////////////////////////get_bitpak //////////////    
    ...
    { 
        ...
        if(!empty($result))
        { 
            $this->response(true,"Bit Packs Fetch Successfully.",array("BitPack List" => $result));         
        }else{
            $this->response(false,"Bit Packs  Not Found",array("BitPack List" => array()));
        }
   }
////////// donate_bit /////////////////////////
  ...
  public function fanProfile()
    {
       ...
       ";
        ...
        $this->db->from('user as U');
        $this->db->join('genre_category as G', 'G.genre_id = U.genre_cat','LEFT');
        $this->db->where('U.id',$_POST['fans_id']);
        $query = $this->db->get();
...
////////////////////////Concert booked by user ////////////////////
   public function concertbooked_byuser()
   {
      $user_id = $_POST['user_id'];
       $this->db->select('C.*');
        $this->db->from('concert_booking as CB');
         $this->db->join('concerts as C', 'C.concert_id = CB.id');
        $this->db->where('CB.user_id',$user_id);
        $query = $this->db->get();
        $detail =  $query->result_array();
        if($detail){
            $this->response(true,"Booking Fetch Successful",array("bookinginfo" => $detail));
        }else{
...
public function get_pinboard()
   {
        ...
        $r=$this->common->getData('post_image',array('post_id'=>$value['id']),array('','field'=>'image'));
         ...
         $this->response(true,"pinboard List",array('pin_list'=>$pin_list)); 
    }else{
...
public function add_pinboard()
  {
     if(!empty($_POST['user_id'])){
        ...
        $this->response(true,"pinboard successfully added");
  }
